...
src_file = pd.read_csv(f"{path_to_tgt}/rxnorm-unmapped-str.csv")
apirxnav.batch_write_rx_code_json(
    path_to_save = path_to_tgt, #absolute path,
    filename_to_save = 'rxnorm-unmapped-str_output',
    sterms = src_file['RAW_RX_MED_NAME'],
    sterm_type = 'string',
    verbose=True
)
...
# df_merge.to_csv(f"{path_to_tgt}/rxnorm-unmapped-str-ref.csv",index = False)